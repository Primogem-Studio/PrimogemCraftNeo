...
public class PGCJEIPlugin implements IModPlugin {
    ...
    public void registerAdvanced(IAdvancedRegistration registration) {
        registration.addTypedRecipeManagerPlugin(WISH, new WishRecipeGenerator());
        registration.addTypedRecipeManagerPlugin(WISH_MATERIALS, new WishMaterialRecipeGenerator());
    }

    @Override
    public void onRuntimeAvailable(IJeiRuntime runtime) {
        WishLootItems.setUpdateListener(() -> runtime.getRecipeManager().unhideRecipes(WISH, List.of()));
        runtime.getRecipeManager().unhideRecipes(WISH, List.of());
    }

    @Override
    public void onRuntimeUnavailable() {
        WishLootItems.setUpdateListener(() -> {});
    }
}
